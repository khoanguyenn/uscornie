    ...
    <div className="flex flex-wrap gap-1.5 mb-5 w-full max-w-full">
      {categories.map((x) => (
        <button
          key={x.id}
          className={cn(
            "flex items-center justify-center gap-1.5 py-2 px-4.5 rounded-[20px] border-2 font-semibold text-[0.8rem] leading-none transition-all duration-250 cursor-pointer shrink-0",
            value === x.id
