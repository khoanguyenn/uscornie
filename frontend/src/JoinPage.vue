...
const handleGoogleResponse = async (response) => {
  ...
  try {
    ...
    if (invite_token) {
        const joinRes = await axios.post('/spaces/join', 
          { invite_token },
...
<template>
  <div class="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-pink-50 to-white p-6">
    <div class="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 text-center transition-all duration-500">
      <div v-if="status === 'welcome'" class="space-y-6">
        <div class="text-7xl">🏠❤️</div>
        <h1 class="text-3xl font-extrabold text-slate-800">Uscornie</h1>
        <p class="text-slate-500 text-lg">Bạn nhận được một lời mời tham gia ngôi nhà chung từ người ấy.</p>
        <div class="flex justify-center mt-8">
            <div id="google-btn"></div>
        </div>
        <p class="text-xs text-slate-400 mt-4 italic">Tiến trình kết nối tức thì, không cần gõ phím.</p>
      </div>
      ...
      <div v-if="status === 'success'" class="space-y-6">
        ...
        <button @click="router.push('/')" class="mt-4 px-6 py-2 bg-pink-500 text-white rounded-full font-bold hover:bg-pink-600 transition">
            Vào nhà ngay
        </button>
      </div>
