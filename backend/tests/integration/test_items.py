...
def test_api_bulk_create_items_success(client: TestClient, db: Session):
    # Setup User and space
    user = User(email="tester_bulk@example.com", full_name="Tester Bulk")
    db.add(user)
    db.commit()
    ...
    payload = {
        "items": [
            {"category": "wishlist", "title": "Bulk Item 1", "desc": "Desc 1"},
            {"category": "movies", "title": "Bulk Item 2", "desc": "Desc 2"},
        ]
    }
    response = client.post(
        f"/spaces/{space.id}/items/bulk",
        json=payload,
        headers=headers,
    )
    assert response.status_code == 201
    data = response.json()
    assert len(data) == 2
    assert data[0]["title"] == "Bulk Item 1"
    assert data[1]["title"] == "Bulk Item 2"

    # Confirm they exist in DB
    items_in_db = db.query(Item).filter(Item.space_id == space.id).all()
    assert len(items_in_db) == 2


def test_api_bulk_create_items_rollback_on_error(client: TestClient, db: Session):
    # Setup User and space
    user = User(email="tester_rollback@example.com", full_name="Tester Rollback")
    db.add(user)
    db.commit()
    ...
    payload = {
        "items": [
            ...
            {"category": "movies", "title": "   ", "desc": "Desc 2"},
        ]
    }
    response = client.post(
        f"/spaces/{space.id}/items/bulk",
    ...
    # Verify no items were created in DB
    items_in_db = db.query(Item).filter(Item.space_id == space.id).all()
